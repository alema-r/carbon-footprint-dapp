...
class SupplierTest(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.transformer_address = Web3.toChecksumAddress(
            "0x0fbdc686b912d7722dc86510934589e0aaf3b55a"
        )
        cls.sup.set_account_as_default(
            user_role=1, address="0xca843569e3427144cead5e4d5999a3d0ccf92b8e"
        )
        cls.client_address = Web3.toChecksumAddress(
        ...
        for i in range(30, 43):
            ...
            cls.raw_materials_list.append(
                RawMaterial(nome, lotto, sup, cf, transformer_address=tr)
            )

        cls.raw_material1 = RawMaterial(
            "MateriaPrima11",
            0,
            cls.web3.eth.default_account,
            20,
            transformer_address=cls.transformer_address,
        )
        cls.raw_material2 = RawMaterial(
            "",
            2,
            cls.web3.eth.default_account,
            20,
        ...
        cls.raw_material3 = RawMaterial(
            "MateriaPrima12",
            0,
            cls.web3.eth.default_account,
            20,
            transformer_address=cls.client_address,
        )
        cls.raw_material4 = RawMaterial(
            "MateriaPrima13",
            0,
            cls.web3.eth.default_account,
            0,
    ...
    def test_create_raw_materials(self):
        self.assertTrue(
            self.sup.create_raw_materials_on_blockchain(self.raw_materials_list)
        )
    ...
    def test_misues1_creation_raw_material(self, mock_stdout):
        self.assertFalse(
            self.sup.create_raw_materials_on_blockchain([self.raw_material2])
        )
        self.assertEqual(
            mock_stdout.getvalue(),
            "execution reverted: One or more raw material has an empty name\n",
        )
    ...
    def test_misues2_creation_raw_material(self, mock_stdout):
        self.assertFalse(
            self.sup.create_raw_materials_on_blockchain([self.raw_material4])
        )
        self.assertEqual(
            mock_stdout.getvalue(),
            "execution reverted: One or more raw material has a carbon footprint value equal to 0\n",
        )
    ...
    def test_misues3_creation_raw_material(self, mock_stdout):
        self.assertFalse(
            self.sup.create_raw_materials_on_blockchain(
                [self.raw_material5, self.raw_material6]
            )
        )
        self.assertEqual(
            mock_stdout.getvalue(),
            "execution reverted: This lot of this raw material has been already inserted. Insertion failed.\n",
        )
    ...
    def test_misuse4_creation_raw_material(self, mock_stdout):
        self.assertFalse(
            self.sup.create_raw_materials_on_blockchain([self.raw_material3])
        )
        self.assertEqual(
            mock_stdout.getvalue(),
            "execution reverted: One or more inserted addresses are not transformer addresses\n",
        )
    ...
    def test_overflow_creation_raw_material(self, mock_stdout):
        self.assertFalse(
            self.sup.create_raw_materials_on_blockchain([self.raw_material9])
        )
        self.assertEqual(
            mock_stdout.getvalue(),
